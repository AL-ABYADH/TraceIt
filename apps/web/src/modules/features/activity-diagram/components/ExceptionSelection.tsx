...
//   onExceptionSelect,
// }: ExceptionSelectionProps) {
//   // The hook is already enabled by !!useCaseId, but let's add console logging to debug
//   const { data: exceptions, isError, isLoading, error } = useExceptions(useCaseId);
...
//             </p>
//             <p className="text-sm text-muted-foreground/70">
//               {isLoading ? "Please wait..." : "Create exceptions first to add them to decision nodes"}
//             </p>
//           </div>
//         )}
//       </div>
...
export default function ExceptionSelection({
  ...
  // Pass enabled option to only fetch when dialog is open
  const {
    data: exceptions,
    isError,
    isLoading,
    error,
  } = useExceptions(useCaseId, {
