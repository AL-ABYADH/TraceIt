...
interface ConditionSelectionDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onCreateNew: () => void;
  onPickExisting: () => void;
...
export default function ConditionSelectionDialog({
  isOpen,
  onClose,
  onCreateNew,
  onPickExisting,
}: ConditionSelectionDialogProps) {
  return (
    <Dialog isOpen={isOpen} onClose={onClose} title="" className="max-w-md p-0 overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between p-6 pb-4">
        <div>
          <h2 className="text-lg font-semibold text-foreground">Add Condition</h2>
          <p className="text-sm text-muted-foreground mt-1">
            Choose how you want to add a condition
          </p>
        </div>
        <button
          onClick={onClose}
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-muted-foreground mt-1 leading-relaxed">
              Create a custom condition from requirements and name it
            </p>
          </div>
        </button>
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-medium text-foreground group-hover:text-green-600 transition-colors">
              Pick Existing Condition
            </h3>
            <p className="text-sm text-muted-foreground mt-1 leading-relaxed">
              Select from existing conditions in the database
            </p>
          </div>
        </button>
      </div>
