...
// }
import type { NodeProps } from "@xyflow/react";
import { Handle, Position, useUpdateNodeInternals } from "@xyflow/react"; // ADDED
import { useMemo, useState, useRef, useEffect } from "react";
import { ActivityShape } from "@/modules/features/activity-diagram/components/ActivityShape";
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem } from "@/components/ui/dropdown-menu";
import { route } from "nextjs-routes";
...
export default function ActivityNode({ data, selected }: NodeProps<any>) {
  ...
  const useCaseId = searchParams.get("useCaseId");
  const router = useRouter();
  const updateNodeInternals = useUpdateNodeInternals(); // ADDED
  ...
  const updateRequirementLabelsMutation = useUpdateRequirementLabels(data?.id, data?.useCaseId, {
    onSuccess: () => {
      ...
      // ADDED: Force edge reconnection after label update completes
      if (data?.id) {
        setTimeout(() => {
  ...
  // ADDED: Force edge reconnection when node size changes with delay
  useEffect(() => {
    if (data?.id) {
  ...
  }, [svgWidth, svgHeight, data?.id, updateNodeInternals]);
  ...
  // FIXED: Consistent handle positioning that works with dynamic widths
